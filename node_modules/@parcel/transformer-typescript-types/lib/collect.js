...
function collect(moduleGraph, context, sourceFile) {
  ...
  let visit = node => {
    if (_typescript().default.isBundle(node)) {
      return _typescript().default.updateBundle(node, _typescript().default.visitNodes(node.sourceFiles, visit));
    }
    ...
    if ((0, _utils.isDeclaration)(_typescript().default, node)) {
      if (node.name) {
        currentModule.addLocal(node.name.text, node);
      }

      let name = (0, _utils.getExportedName)(_typescript().default, node);
